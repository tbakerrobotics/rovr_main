...
/**
 * Struct that holds per-participant data for callback.
 */
typedef struct {
...
/**
 * Called before an audio processing unit mixes the per-participant audio data to a single stream, from the audio processing thread.
 * No blocking operations should occur on this callback;
 */
