...
typedef struct vx_req_account_login {
    ...
    /**
     * This is an integer that specifies the maximum number of participant property events the SDK will send while in a channel.
     * If set the SDK will attempt to generate an event at the given frequency, but will not generate two identical events in a row.
     * If this is not set the default will be "on state change", which means that the events will be sent when the participant
     * starts talking, stops talking, is muted, is unmuted.
     *
...
typedef struct vx_req_sessiongroup_add_session {
    ...
    /**
     * Either returned in vx_resp_sessiongroup_create from successful creation of a session group or
     * a string representing the preferred session group handle. If empty, one will be generated automatically.
     * Preferred handles must include at least one non numeric character.
     */
    VX_HANDLE sessiongroup_handle;
 ...
 *
 * The XML for this request can be found here: \ref Session_SetLocalRenderVolume_1
 *
...
#ifndef VIVOX_TYPES_ONLY
/**
 * Used to allocate and initialize a vx_req_session_set_local_render_volume object
 * \ingroup session
 */
...
/**
 * Used to set the volume for a participant in the current session.
